...
class DKVMNAnalyzer():
    ...
    def test1_base(self, answer_type, update_value_matrix_flag):
        ...
        for action_idx in range(self.num_actions):
            ...
            for th_idx, th in enumerate(th_list):
                if answer_type == 1:
                    wrong_response[th_idx] = np.sum(probs_diff < -th)
                ...
                if wrong_response[th_idx] == 0:
                    right_updated_skill_counter[th_idx] += 1
        ...
        self.logger.info('{}'.format(self.int2str_list(wrong_updated_skill_list)))
    ...
    def test2_base(self, answer_type, update_value_matrix_flag):
        ...
        self.logger.info(self.float2str_list(diff))
        self.logger.info(self.float2str_list(rmse))
    ...
    def float2str_list(self, input_list):
        return ','.join('{:4f}'.format(item) for item in input_list)

    def int2str_list(self, input_list):
